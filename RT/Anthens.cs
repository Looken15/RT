...
namespace RT
{
    ...
    static class Athens
    {
        ...
        public static void AtheneTransform(ref Mesh mes, double[,] m2, bool rotation = false)
        {
            ...
            foreach (Edge e in mes.edges)
            {
                ...
                m = MatrixMult(m1, m2);
                newp = new Point3D((float)m[0, 0], (float)m[0, 1], (float)m[0, 2]);
            ...
            foreach (Polygon p in mes.faces)
            {
                ...
                if (rotation)
                {
                    double[,] m1n = new double[1, 4] {
                        { p.normal.X, p.normal.Y, p.normal.Z, 1 } 
                    };

                    double[,] mn = MatrixMult(m1n, m2);
                    Point3D newpn = new Point3D((float)mn[0, 0], (float)mn[0, 1], (float)mn[0, 2]);
